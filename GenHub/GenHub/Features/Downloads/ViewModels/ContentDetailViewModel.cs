...
public partial class ContentDetailViewModel : ObservableObject
{
    ...
    /// <summary>
    /// Gets a value indicating whether articles are available.
    /// </summary>
    ...
    public ObservableCollection<WebFile> Files
    {
        get
        {
            ...
            if (files.Count == 0 && _searchResult.ResolverMetadata.TryGetValue(GenHub.Core.Constants.GitHubTopicsConstants.HasVariantsMetadataKey, out var hasVariants)
                && bool.TryParse(hasVariants, out var hasVariantsBool) && hasVariantsBool)
            {
                var variants = _searchResult.GetData<List<GenHub.Core.Models.GitHub.GitHubVariantArtifact>>(GenHub.Core.Constants.GitHubTopicsConstants.VariantsJsDataKey);
                if (variants != null)
                {
    ...
    public bool HasGitHubVariants =>
        _searchResult.ResolverMetadata.TryGetValue(GenHub.Core.Constants.GitHubTopicsConstants.HasVariantsMetadataKey, out var hasVariants)
    ...
    /// <summary>
    /// Gets a value indicating whether there are any tags to display.
    /// </summary>
    ...
    public void PopulateReleases(IEnumerable<WebFile> files)
    {
        ...
        foreach (var file in files.Where(f => f.FileSectionType == FileSectionType.Downloads))
        {
            ReleaseItemViewModel releaseItem = new()
            {
                ...
                // Wire up commands
                DownloadCommand = new AsyncRelayCommand(async () => await DownloadFileAsync(file)),
                AddToProfileCommand = new AsyncRelayCommand(async () => await AddFileToProfileAsync(file)),
            };
