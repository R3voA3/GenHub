    ...
    ILogger<ContentStateService> logger) : IContentStateService
{
    ...
    private string GetProspectiveId(ContentSearchResult item)
    {
        ...
        return ManifestIdGenerator.GeneratePublisherContentId(
            item.ProviderName ?? "unknown",
            item.ContentType,
            string.IsNullOrWhiteSpace(item.Name) ? (item.Id ?? "unknown") : item.Name,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            _logger.LogWarning("Failed to retrieve manifests for matching: {Error}", allManifestsResult.FirstError);
            return (null, false);
        }
