    ...
    ILogger<SubscriptionConfirmationViewModel> logger) : ObservableObject
{
    ...
    public async Task InitializeAsync()
    {
        try
        {
            ...
            using var cts = new CancellationTokenSource(TimeoutConstants.CatalogRefresh);
        ...
        catch (OperationCanceledException)
        {
            ...
            ErrorMessage = $"Failed to fetch catalog: Request timed out ({GenHub.Core.Constants.TimeoutConstants.CatalogRefresh.TotalSeconds}s). Please check your connection and try again.";
        }
        catch (Exception ex)
        {
