...
public class JsonPublisherCatalogParser(ILogger<JsonPublisherCatalogParser> logger) : IPublisherCatalogParser
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var byteCount = System.Text.Encoding.UTF8.GetByteCount(catalogJson);
            if (byteCount > ContentConstants.MaxCatalogSizeBytes)
            {
                return OperationResult<PublisherCatalog>.CreateFailure($"Catalog JSON exceeds maximum size limit ({ContentConstants.MaxCatalogSizeBytes} bytes)");
            }
