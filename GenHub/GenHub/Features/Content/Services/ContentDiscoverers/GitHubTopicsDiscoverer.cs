    ...
    ILogger<GitHubTopicsDiscoverer> logger) : IContentDiscoverer
{
    ...
    private static readonly string[] _standaloneExtensions = [".big", ".csf", ".ini", ".w3d", ".dds", ".tga"];
    ...
    private static bool ShouldSplitAssets(GitHubRelease release)
    {
        ...
        var standaloneCount = contentAssets.Count(a =>
            _standaloneExtensions.Any(ext => a.Name.EndsWith(ext, StringComparison.OrdinalIgnoreCase)));
    ...
    private static bool HasVariantPattern(List<GitHubReleaseAsset> assets)
    {
        ...
        var languageMatches = assets
            ...
            .SelectMany(name => languagePatterns.Where(lang => name.Contains(lang, StringComparison.OrdinalIgnoreCase)))
            .Distinct()
            .ToList();
        ...
        string sourceTopic)
    {
        ...
        if (latestRelease != null && ShouldSplitAssets(latestRelease))
        {
            ...
            result.ResolverMetadata[GitHubTopicsConstants.HasVariantsMetadataKey] = "true";
            result.ResolverMetadata[GitHubTopicsConstants.VariantCountMetadataKey] = contentAssets.Count.ToString();

            // DownloadSize is already filtered in CreateSearchResult
            ...
            result.SetData(GitHubTopicsConstants.VariantsJsDataKey, variants);
        }
        ...
        string sourceTopic)
    {
        ...
        var result = new ContentSearchResult
        {
            ...
            DownloadSize = latestRelease?.Assets?.Where(a => !IsSourceCodeAsset(a.Name)).Sum(a => a.Size) ?? 0,
