...
public class ContentOrchestrator : IContentOrchestrator
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        void ReportProgress(int stage, string description, double stageProgress = 0, string? operation = null, bool isBottleneck = false, string? bottleneckReason = null, long bytesProcessed = 0, long totalBytes = 0, int filesProcessed = 0, int totalFiles = 0, string? currentFile = null)
        {
        ...
        try
        {
            ...
            try
            {
                ...
                var downloadProgress = new Progress<ContentAcquisitionProgress>(p =>
                {
                    ...
                    ReportProgress(2, "Downloading", stagePercent, operation, bytesProcessed: p.BytesProcessed, totalBytes: p.TotalBytes, filesProcessed: p.FilesProcessed, totalFiles: p.TotalFiles, currentFile: p.CurrentFile);
                });
                ...
                var totalBytes = prepareResult.Data.Files.Sum(f => f.Size);
                ReportProgress(2, "Downloading", 100, "Download complete", bytesProcessed: totalBytes, totalBytes: totalBytes, filesProcessed: prepareResult.Data.Files.Count, totalFiles: prepareResult.Data.Files.Count);
