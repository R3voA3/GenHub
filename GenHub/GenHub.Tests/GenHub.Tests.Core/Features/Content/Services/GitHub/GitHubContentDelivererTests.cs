...
public class GitHubContentDelivererTests
{
    ...
    public async Task DeliverContentAsync_ShouldExtractZip_ForMatchingContentTypes(ContentType contentType)
    {
        ...
        try
        {
            ...
            // Verify that the factory was actually called to create manifests from extracted content
            mockFactory.Verify(
                f => f.CreateManifestsFromExtractedContentAsync(
                    It.Is<ContentManifest>(m => m.Id == manifest.Id),
                    It.Is<string>(d => d == tempDir),
